...
| `宏被禁用`（运行时报） | 文件受保护视图（网络下载）。**先审后解**：先 `export_vba_source` 导出源码人工审阅（重点 `Declare`/`Shell`/`CreateObject("WScript...")`），确认无害再让用户右键→属性→解除锁定；run 系列已自动用 Low 安全级（工具对带下载标记的文件会直接拒绝，见 SKILL.md「不可信文件红线」） |
...
| 文件被锁定（Open 失败） | 其他 Excel 实例占用 | 已装插件被用户 Excel 加载 → `is_file_locked()` 确认后 `unload_addin()` 释放锁（改完 `reload_addin()`）；工具自身残留实例 → `shutdown_excel()`；用户打开了同名工作簿 → 请用户关闭，或 `attach=True`（attach 下运行宏不做超时强杀，卡死需人工结束）；用户 Excel 停在 Backstage（未开任何工作簿）→ attach 不上，请用户先打开任意工作簿或手动取消加载项勾选 |
...
| 导出的 .bas/.cls 中文乱码 | 旧版工具导出为 GBK | 2026-09 起导出/导入统一 UTF-8（导入自动兼容旧 GBK 文件）；`.frm` 本身是文本（头+代码）但伴生 `.frx` 是二进制——仍不要文本直改窗体，改布局走 COM API |
